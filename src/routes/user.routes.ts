...
router.put("/:id", async (req: Request<{ id: string }>, res: Response, next: NextFunction) => {
    try {
        ...
        if (password) {
            userToUpdate.password = await bcrypt.hash(password, 10); // Hash the updated password securely
        }

        await userRepository.save(userToUpdate);

        const { password: _, ...sanitizedUser } = userToUpdate; // Remove password from response
